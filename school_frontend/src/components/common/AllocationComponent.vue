<template>
  <div v-if="classes && classes.length > 0">
        ...
        <div>
          <h4 v-if="selectedSubjects.length > 0">{{$globalData.teacherData.first_name}} {{$globalData.teacherData.last_name}} Allocations</h4>
        <!-- Show the table with subjects for the active tab -->
  ...
  methods: {
    ...
    async submitAllocation() {
      ...
      try {
        ...
        const allocations = this.selectedSubjects.map((subjectObj) => ({
    ...
    await this.postMethod('/subject_allocation', { allocation: allocations }, '/subject_allocation');
